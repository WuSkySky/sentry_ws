...
def generate_launch_description():
    ...
    if launch_params['video_play']: 
        ...
        #     extra_arguments=[{'use_intra_process_comms': True}]
        # )
        cam_type = launch_params.get('camera_type', 'daheng')
        if cam_type == 'hik':
            image_node = ComposableNode(
                package='hik_camera', 
                plugin='hik_camera::HikCameraNode', 
                name='camera_driver',
                parameters=[get_params('hik_camera')], # 加载 hik_camera_params.yaml
                extra_arguments=[{'use_intra_process_comms': True}]
            )
        else:
            image_node = ComposableNode(
                ...
                plugin='fyt::camera_driver::DahengCameraNode',
                name='camera_driver',
                parameters=[get_params('daheng_camera')], # 加载 daheng_camera_params.yaml
                extra_arguments=[{'use_intra_process_comms': True}]
            )
