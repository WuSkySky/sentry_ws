...
void ProtocolInfantry::send(const rm_interfaces::msg::GimbalCmd &data) {
  ...
  // 显式填充头尾
  packet.loadData<unsigned char>(0xFF, 0);   // Index 0: Header
  packet.loadData<unsigned char>(0x0D, 15);  // Index 15: Tail
  ...
  packet.loadData<float>(static_cast<float>(data.distance), 10);
    uint8_t buffer[16];
  // 按照上面 loadData 的逻辑手动填一遍 buffer，用来算校验（有点笨但安全）
  buffer[0] = 0xFF;
  buffer[1] = data.fire_advice ? FireState::Fire : FireState::NotFire;
  // float 拷贝
  float pitch = static_cast<float>(data.pitch);
  float yaw = static_cast<float>(data.yaw);
  float dist = static_cast<float>(data.distance);
  memcpy(&buffer[2], &pitch, 4);
  memcpy(&buffer[6], &yaw, 4);
  memcpy(&buffer[10], &dist, 4);
  ...
  // 3. 将算好的校验位 load 进去
  packet.loadData<unsigned char>(checksum, 14); // Index 14: Checksum
