...
bool FixedPacketTool<capacity>::checkPacket(uint8_t *buffer, int recv_len) {
  ...
  if ((buffer[0] != 0xff) || (buffer[capacity - 1] != 0x0d)) {
    ...
    // 3. 【新增】检查校验位 (BCC 异或校验)
  // 校验范围：从 buffer[0] 到 buffer[capacity-3] (即 index 0 到 13)
  // 校验位位置：buffer[capacity-2] (即 index 14)
  uint8_t calculated_sum = 0;
  // 遍历数据段（包含帧头，但不包含校验位本身和帧尾）
  for (int i = 0; i < capacity - 2; i++) {
    calculated_sum ^= buffer[i];
  }
  // 对比算出来的 sum 和接收到的校验位
  if (calculated_sum != buffer[capacity - 2]) {
    // 校验失败（数据被篡改或损坏）
    FYT_WARN("serial_driver", "Checksum failed! calc:%02x recv:%02x", calculated_sum, buffer[capacity-2]);
    return false;
  }
